function [data] = finalBehavior(subjectID, task, tool, order)
% finalBehavior('1', 'S', 'E', 1);
%
% task is  'S', 'F' or 'B' (self, familiar, both)
% tool is 'B' or 'E' (bone-conduction, earphones)
% order is block order (1 - 6)
...
fileName = ['FinalBeh_S' num2str(subjectID) '_'  task '_' tool '_' num2str(order)];
...
try 	
    ...
    if ~(task == 'S' || task == 'F' || task == 'B' )  
		disp('Task has to be ''S'' or ''F'' or ''B''! ');
		data = task;
		return
    end 
    
    
    levels = [15 30 45 55 70 85];  % percentage of subject's voice
